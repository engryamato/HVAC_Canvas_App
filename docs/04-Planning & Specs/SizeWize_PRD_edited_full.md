SizeWise HVAC Canvas - Product Requirements Document
(PRD)
Version: 1.1.0
Date: 2026-02-16
...
1. Executive Summary
1.1 Product Vision
SizeWise HVAC Canvas is a professional-grade, desktop-first HVAC design and estimation application that
enables HVAC professionals to design ventilation systems, perform engineering calculations, and generate
accurate bills of materials.
1.2 Target Users
User Type                Description                               Primary Use Cases
Prepares quotes for commercial HVAC
HVAC Estimator                                                     Create estimates, generate BOMs, export pricing sheets
projects
HVAC Designer            Designs ventilation systems for buildings Layout ductwork, size equipment, verify airflow
Kitchen Ventilation      Focuses on commercial kitchen exhaust     Design hood systems, calculate makeup air, meet code
...
7. Bill of Materials generation with CRM-compatible CSV export
8. Basic Quoting System with configurable pricing and export scope
9. PDF export for professional documentation
1.4 Success Metrics
...
FAB Tool: Floating Action Button triggered by Spacebar (or F key) for quick entity creation. The 'D' key remains dedicated to activating the Duct drawing tool.
FR-UI-002: Left Sidebar (Project Context)
Project Details (Accordion):
Name
Project ID / Case Number (CRM Link)
Location
Client
...
Trigger: Press Spacebar (or F key). The 'D' key is reserved for the Duct drawing tool.
Umbrella Menu:
...
Detection: Automatically detect device type (PC, Laptop, Tablet, Mobile).
Behavior: On devices with screen width < 768px (phones/small tablets), enter read-only mode. Users can view projects, drawings, and BOMs but editing actions are disabled. Display a message: "Switch to desktop to edit."
...
Auto-Connect: Automatically insert elbows/tees when drawing ducts.
Smart Selection: Choose correct fitting type based on geometry.
...
- Kitchen Exhaust: 500 FPM (Min) – 2,500 FPM (Recommended) – 4,000 FPM (Max)
- Generator Exhaust: 2,000-5,000 FPM
...
1. Reynolds Number:
Re = (D × V) / (12 × ν)
(ν = Kinematic viscosity of air ≈ 1.6 × 10^-4 ft²/s)
...
ΔP = f × (L/D) × (V/4005)²
Where:
ε = Absolute roughness (ft) of material
L = Duct length (ft)
D = Duct diameter (ft)
V = Velocity (FPM)
...
| Aluminum | 0.0002 |
| Flex duct | 0.003 |
Note: Use ε (Absolute roughness) from the table above in the Swamee-Jain friction factor formula.
...
Industrial Mode (Exhaust/Grease):
...
FR-CALC-010: Return Air Calculations
Support return air CFM calculations at room and system level
Allow user to designate ducts/equipment as Return
Validate return path continuity (room → return ductwork → return fan/AHU)
Include return airflow results in the Calculations panel
FR-CALC-011: Variable Air Volume (VAV) Systems
Support VAV terminals as equipment entities with:
Minimum airflow (CFM)


Maximum airflow (CFM)
Control setpoint mode (Cooling, Heating, Auto)
Support supply duct sizing based on design airflow scenario:
Peak (max airflow)
Minimum (min airflow)
Diversity factor (system-level override)
Include VAV sizing and validation results in the Calculations panel
Include VAV-related line items in BOM and Quote outputs when applicable
...
id: string; category: 'duct' | 'fitting' | 'equipment';      subcategory: string;
// e.g., "Round Duct", "90° Elbow" description: string;              // e.g., "12\" Galva
nized Round Duct" quantity: number; unit: 'LF' | 'EA' | 'SF';    // Linear feet, Ea
ch, Square feet material?: string; size?: string; entityIds: string[];          //
Source entity IDs}
2.8 File Management
...
{ "schemaVersion": "1.0.0", "projectId": "uuid", "projectName": "string", "crea
tedAt": "ISO8601", "modifiedAt": "ISO8601", "entities": {}, "viewportState": {},
"settings": {}}
...
FR-EXPORT-002: CSV Export (BOM & CRM)
Export Bill of Materials as CSV
Format optimized for CRM import (Dynamics 365, generic CRM)
Columns: Case ID, Category, Subcategory, Description, Quantity, Unit, Size, Material, Unit Price, Total Price
Note: Unit Price is derived from a static base price specified in each component. Total Price = Quantity × Unit Price.
UTF-8 with BOM for Excel compatibility
Filename: {projectName}_BOM_{date}.csv
FR-EXPORT-003: PDF Export
Generate professional PDF document
Scope Selection Dialog: User selects components to include:
Drawing / Canvas
Bill of Materials (BOM)
Formal Quote / Pricing
...
FR-EXPORT-004: Export Scope Configuration
Dialog appears before any export (PDF/CSV)
Checkbox options:
Include Drawing
Include BOM
Include Quote
...
Default Assignment: Assign default “General Supply” service to legacy entities lacking service context
execution.
...
Calculation Defaults: Use conservative defaults (e.g., standard friction factors) if specific engineering data is
missing from legacy files.
2.12 Quoting System
FR-QUOTE-001: Pricing Configuration
Input base costs for material categories (e.g., per lb, per ft)
Configure markup percentages
Set labor rates (per hour) and installation time factors
FR-QUOTE-002: Quote Generation
Calculate total project cost based on BOM quantities
Apply markups and labor calculations
Generate line-item pricing
3. Technical Requirements
3.1 Technology Stack
Component            Technology       Version          Purpose
Desktop Runtime      Tauri            2.x              Native desktop wrapper, filesystem access
Frontend
Next.js          16.x             React framework with routing
Framework
...
Pure Canvas
Canvas Rendering                      -                No wrapper libraries; Fabric.js explicitly rejected per DEC-001
2D
Browser-based persistence for web; .sws file remains authoritative
...
Requirement: Without third-party wrapper libraries, implement a spatial indexing structure (e.g., Spatial Hash Grid or Quadtree) to handle hit detection and object selection efficiently. This ensures interactive performance (60 fps) when managing hundreds of entities on a pure 2D canvas.
3.2 Architecture Patterns
3.2.1 Command Pattern (State Mutations)
All state changes MUST be executed via commands:
interface Command {
id: string;              // ULID for ordering                     type: string;            // e.g., "CR
EATE_ROOM", "UPDATE_DUCT" payload: unknown;                            // Command-specific data timestam
p: number;       // Unix timestamp}
...
id: string;                        // UUID v7
type: EntityType;                  // 'room' | 'duct' | 'equipment' | 'fitting' | 'note' | 'gr
oup'
transform: Transform;
...
interface Transform {
x: number;              // Pixels from origin    y: number;   rotation: number;
// Degrees scaleX: number; scaleY: number;}
3.3.2 Room Entity
interface Room extends BaseEntity {
type: 'room'; props: {
name: string;                    // 1-100 chars     width: number;
// inches, 1-10,000    length: number;                   // inches, 1-10,000    height:
number;                   // inches, 1-500    occupancyType: OccupancyType; airChang
esPerHour: number;        // 1-100    notes?: string; }; calculated: {
area: number;                    // sq ft    volume: number;               // c
u ft     requiredCFM: number; };}
type OccupancyType = | 'office' | 'retail' | 'restaurant' | 'kitchen_commercial'
| 'warehouse' | 'classroom' | 'conference' | 'lobby';
3.3.3 Duct Entity
interface Duct extends BaseEntity {
type: 'duct'; props: {
name: string;    shape: 'round' | 'rectangular';    diameter?: number;
// round only, inches     width?: number;                  // rectangular only, inches
height?: number;                  // rectangular only, inches     length: number;
// feet     material: DuctMaterial;    airflow: number;                  // CFM (canonic
al field name)     staticPressure: number;          // in.w.g.     connectedFrom?: strin
g;           // Entity ID    connectedTo?: string;             // Entity ID }; calcula
ted: {
area: number;                    // sq in    velocity: number;                    // F
PM     frictionLoss: number;           // in.w.g./100ft };}
...
name: string;    equipmentType: EquipmentType;     manufacturer?: string;       modelN
umber?: string;    capacity: number;                // CFM     staticPressure: number;
// in.w.g.    width: number;                   // inches     depth: number;
// inches    height: number;                  // inches };}
type EquipmentType = 'hood' | 'fan' | 'diffuser' | 'damper';
3.3.5 Fitting Entity
interface Fitting extends BaseEntity {
type: 'fitting'; props: {
fittingType: FittingType;    angle?: number;                    // degrees (for elbo
ws)    inletDuctId?: string;    outletDuctId?: string; }; calculated: {
equivalentLength: number;        // feet    pressureLoss: number;               // i
n.w.g. };}
...
// Pricing
pricing: {
basePrice: number;                // Static unit price for exports
materialCost: number;
...
type: 'galvanized_steel' | 'stainless_steel' | 'aluminum' | 'fiberglass' | 'flexibl
e';
...
// Broader enum set (fresh_air, relief_air, other) is a conscious deviation from ori
ginal plan for completeness
systemType: 'supply' | 'return' | 'exhaust' | 'fresh_air' | 'relief_air' | 'other';
material: 'galvanized' | 'stainless' | 'aluminum' | 'fiberglass' | 'flexible' | 'carbon_steel' | 'black_iron' | 'aluminized_steel' | 'double_wall_insulated';
pressureClass: 'low' | 'medium' | 'high' | 'low-pressure' | 'medium-pressure' | 'hig
h-pressure';
...
// Metadata
source: 'baseline' | 'custom';          // 'baseline' = system template, 'custom' = user
-created
...
id: string;                         // Static string IDs like 'tmpl_low_pressure_sup
ply'
isTemplate: true;
}
3.3.8 Note Entity
...
Note: Import/export functionality is handled by separate services, not directly by the store. See
ComponentImportService for CSV/JSON import operations.
3.4.2 Service Store
Store: serviceStore
...
Note: Service assignment to entities is handled through the entity’s serviceId property in the entity store, not
through a store action. Use entityStore.updateEntity(entityId, { serviceId: '...' }) to assign services.
...
schemaVersion: string;                              // Semantic version     projectId: string;
// UUID       projectName: string;            projectNumber?: string;      clientName?: string;    createdA
t: string;                               // ISO8601     modifiedAt: string;                  // ISO8601
entities: {
byId: Record<string, Entity>;                  allIds: string[]; }; viewportState: {
panX: number;    panY: number;                  zoom: number; }; settings: {
unitSystem: 'imperial' | 'metric';                   gridSize: number;      gridVisible: boolean;
};     commandHistory?: {
...
{      "occupancyTypes": {                 "office": {              "Rp": 5,          "Ra": 0.06,            "defaultDens
ity": 0.005,      "defaultACH": 4                         },      "restaurant": {                  "Rp": 7.5,      "Ra":
0.18,      "defaultDensity": 0.015,                             "defaultACH": 10            }      }}
