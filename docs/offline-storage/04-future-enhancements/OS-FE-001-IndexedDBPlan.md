# Offline Storage: IndexedDB Implementation Plan

## 1. Overview

### Purpose
Document the **planned IndexedDB implementation** for future browser-based caching, including why it's mentioned in the PRD, current alternatives, and implementation roadmap.

### Scope
- Why IndexedDB was considered
- Current alternative (localStorage + .sws files)
- When IndexedDB would be beneficial
- Implementation roadmap and considerations
- Migration path from current architecture

### Implementation Status
- **Status**: ❌ Not Implemented (Future Enhancement)
- **Current Alternative**: localStorage via Zustand persist middleware
- **Priority**: Low (current solution works for Phase 1)
...
- **Section 4.2: Offline-First Architecture** - Mentions IndexedDB as potential caching layer
- **PRD.md:431** - IndexedDB mentioned in data persistence strategy

### Why It's in the PRD

IndexedDB was included in the PRD during initial planning as a:
1. **Best practice** for offline-first web applications
2. **Scalable solution** for large datasets (>5MB)
3. **Standard recommendation** for PWA (Progressive Web App) architectures

However, **implementation was deferred** because:
- localStorage + .sws files meet Phase 1 requirements
- IndexedDB adds complexity without immediate benefit
- Small to medium projects fit within localStorage limits

---

## 3. Current Architecture vs IndexedDB
...
graph TB
    ...
    subgraph "Browser Cache"
        LS[localStorage<br/>~5MB limit<br/>Key-value only]
    end

    subgraph "Permanent Storage"
        FS[.sws Files<br/>Unlimited size<br/>Tauri only]
    end

    ZS -->|persist middleware| LS
    ZS -->|manual save/load| FS

    style LS fill:#fff4e1
    style FS fill:#e8f5e9
```

### Future Architecture with IndexedDB
...
graph TB
    ...
    subgraph "Browser Cache"
        ...
        IDB[IndexedDB<br/>~50GB+ limit<br/>Structured queries]
    end
...
## 4. When IndexedDB Would Be Beneficial

### Use Cases for IndexedDB

| Use Case | localStorage | IndexedDB | Winner |
|----------|-------------|-----------|--------|
| **Preferences** (1KB) | ✅ Simple | ❌ Overkill | localStorage |
| **Project list** (10KB) | ✅ Works | ✅ Works | localStorage (simpler) |
| **Single project cache** (500KB) | ⚠️ Works but tight | ✅ No problem | IndexedDB (scalable) |
| **10 project caches** (5MB) | ❌ Exceeds quota | ✅ No problem | IndexedDB |
| **Complex queries** | ❌ Manual filtering | ✅ Indexed queries | IndexedDB |
| **Blob storage** (images) | ❌ Base64 only | ✅ Native blobs | IndexedDB |

### Trigger Points for Implementation

Implement IndexedDB when:

1. **Storage quota exceeded**: Users hit 5MB localStorage limit
2. **Query performance**: Need indexed searches (find projects by client)
3. **Blob storage**: Need to cache PDF plans or thumbnail images
4. **Offline sync**: Multiple devices sync via cloud (IndexedDB tracks changes)
5. **PWA deployment**: Building Progressive Web App with offline-first requirements

**Current Status**: None of these trigger points reached in Phase 1
