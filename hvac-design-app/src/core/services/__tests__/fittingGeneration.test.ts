...
describe('FittingGenerationService', () => {
  ...
  it('records connection point indexes as fitting port indexes, not duct endpoint indexes', () => {
    const activeComponent: UnifiedComponentDefinition = {
      id: 'svc-port-index',
      name: 'Low Pressure Supply',
      componentClass: 'duct',
    ...
    expect(fitting?.props.connectionPoints).toEqual([
      { ductId: 'd-old', pointIndex: 0 },
      { ductId: 'd-new', pointIndex: 1 },
    ]);
  });

  it('allows a transition fitting even when the active service has no reducer rule', () => {
    const activeComponent: UnifiedComponentDefinition = {
      id: 'svc-transition',
      name: 'Supply Without Reducer Rule',
      componentClass: 'duct',
      categoryId: 'standard-duct',
      typeId: 'duct',
      engineeringSystem: 'standard_duct',
    ...
    expect(fitting).not.toBeNull();
    expect(fitting?.props.fittingType).toBe('reducer');
  });
});
